...
class Player extends React.Component {
  render() {
    var rankClass = `${this.props.rank <= 3 ? 'player-rank-' + this.props.rank : ''}`
    return (
    <a className={"player-panel panel-block is-size-3 " + rankClass}>
      ...
      <div className="column is-6-mobile is-5-tablet">
        <div className="has-text-centered is-size-5-mobile is-size-4-tablet is-size-3-desktop points is-flex">
          <span className="icon sub circle-border" onClick={this.props.setScore.bind(this, this.props.id, this.props.score - 1)}>
            <i className="fas fa-minus has-text-danger"></i>
          </span>
          <span><strong>{this.props.score} pts.</strong></span>
          <span className="icon add circle-border" onClick={this.props.setScore.bind(this, this.props.id, this.props.score + 1)}>
